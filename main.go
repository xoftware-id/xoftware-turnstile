...
func main() {
	...
	mux.HandleFunc("/api/turnstile/init", handleInitChallenge)
...
func validateTelemetry(t ClientTelemetry) string {
	if t.IsWebDriver {
		return "Automation framework detected (WebDriver)"
	}
	if t.ScreenWidth == 0 || t.ScreenHeight == 0 {
		return "Invalid screen dimensions (Headless?)"
	}
	if t.WindowOutWidth == 0 || t.WindowOutHeight == 0 {
		return "Invalid window outer dimensions (Headless?)"
	}
	if t.WindowInWidth > t.WindowOutWidth || t.WindowInHeight > t.WindowOutHeight {
		return "Dimension mismatch (Window Inner > Outer)"
	}
	if t.DeviceMemory == 0 && t.CPUCore == 0 {
		return "Device hardware information missing (Bot?)"
	}
	return ""
}

func handleInitChallenge(w http.ResponseWriter, r *http.Request) {
	if r.Method != "POST" {
	...
	if errReason := validateTelemetry(req.Telemetry); errReason != "" {
		w.WriteHeader(http.StatusForbidden)
		json.NewEncoder(w).Encode(ApiResponse{Success: false, Error: errReason})
		return
	}
